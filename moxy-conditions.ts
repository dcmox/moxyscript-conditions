...
export const conditionsValidate = (
	...
	conditions.forEach((cond: ICondition, index: number) => {
		...
		if (keyReplaced.length !== key.length) {
			...
			ret.push({
				index,
				reason: 'Key contains unsafe character',
				valid: false,
			})
			return
		}
		if (typeof value === 'string' && !value.indexOf('`')) {
			...
			ret.push({
				index,
				reason: 'Value contains unsafe character',
				valid: false,
			})
			return
		}
		...
		if (~opKeys.indexOf(operator)) {
			if (typeof value === 'object') {
				...
				if (!value.hasOwnProperty('valueOf')) {
					...
					ret.push({
						index,
						reason: 'Value can only be of type Date | string',
						valid: false,
					})
					return
				}
				...
				if (
					~['string', 'boolean'].indexOf(typeof value) &&
					!~['eq', 'neq'].indexOf(operator)
				) {
					pass = false
					ret.push({
						index,
						reason: `${typeof value} comparisons must only be eq | neq`,
						valid: false,
					})
					return
				}
			...
			if (operator === 'between' && valueB) {
				if (typeof value !== typeof valueB) {
					...
					ret.push({
						index,
						reason: 'Both values must be of same type',
						valid: false,
					})
					return
				}
				if (
					~['string', 'boolean'].indexOf(typeof value) ||
					~['string', 'boolean'].indexOf(typeof valueB)
				) {
					pass = false
					ret.push({
						index,
...
export const conditionsToString = (conditions: ICondition[]): string => {
	...
	conditions.forEach((cond: ICondition) => {
		...
		if (~opKeys.indexOf(operator)) {
			if (typeof value === 'object') {
				// Date support
				evalString += `(new Date(item.${key}.valueOf()) ${
					opMap[operator]
				} ${value.valueOf()}) && `
			} else {
...
export const itemsMatchConditions = (
...
export const itemsFailConditions = (
	items: IKeyValueObject[],
	conditions: string,
) => {
	// tslint:disable-next-line: no-eval
...
export class ConditionEditor {
	...
	public getCondition(name: string): string | boolean {
		return this._condMap[name] ? this._condMap[name] : false
	}

	public runMatchCondition(
		name: string,
		data: any,
	): IKeyValueObject[] | boolean {
		return this._condMap[name]
			? itemsMatchConditions(data, this._condMap[name])
			: false
	}

	public runFailCondition(
		name: string,
		data: any,
	): IKeyValueObject[] | boolean {
		return this._condMap[name]
			? itemsFailConditions(data, this._condMap[name])
			: false
	}
	...
	public saveConditions(): boolean {
		...
		fs.writeFile(
			this.exportPath,
			JSON.stringify(this._condMap, null, 2),
			(err: any) => {
				if (err) {
					throw new Error(err)
				}
